...
function checkInclusion(str1: string, str2: string): boolean {
  ...
  while (right < str2.length) {
    // Step 2: Subtract right-pointer char from map. Delete property if frequency is 0.
    ...
    // Step 3: If window is too long, increment the left pointer.
    ...
    // Step 4: If map is empty (all chars === 0), return true.
    if (map.size === 0) {
      return true;
    }
